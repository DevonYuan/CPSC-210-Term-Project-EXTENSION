package com.speedcubing.backend.model;
...
public class UserBase {
    ...
    public double[] numSolvesStats() {
        ...
        if (numSolvesInOrder.size() % 2 == 0) {
            ...
            stats[3] = findMedian(numSolvesInOrder.subList((size + 1) / 2, size));
        }
    ...
    public JSONObject toJson(){
        ...
        json.put("users", userBaseToJson());
        return json;
    }

    // EFFECTS: Returns users in the userbase as a JSON array
    private JSONArray userBaseToJson(){
        JSONArray jsonArray = new JSONArray();
        users.forEach((u) -> {
            addUserToArray(jsonArray, u);
        });
        return jsonArray;
    }

    // MODIFIES: jsonArray
    // EFFECTS: represents user to the given JSONArray
    private void addUserToArray(JSONArray jsonArray, User user){
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("name", user.getName());
        jsonObject.put("userName", user.getUserName());
        jsonObject.put("password", user.getPassWord());
        jsonObject.put("sessions", sessionsToJSON(user));
        jsonArray.put(jsonObject);
    }

    // EFFECTS: Returns a JSONArray representing a given user's sessions
    private JSONArray sessionsToJSON(User user){
        JSONArray sessions = new JSONArray();
        user.getPracticeSessions().forEach((p) -> {
            sessions.put(sessionToJSON(p));
        });

        return sessions;
    }

    // EFFECTS: Returns a JSON Array representing a given practice session
    private JSONObject sessionToJSON (PracticeSession p){
        JSONObject sessionAsJSON = new JSONObject();
        sessionAsJSON.put("sessionName", p.getSessionName());

        JSONArray solves = new JSONArray();
        p.getSolves().forEach((s) -> {
            JSONObject solve = new JSONObject();
            solve.put("time", s.getTime());
            solve.put("scramble", s.getScramble());
            solves.put(solve);
        });

        sessionAsJSON.put("solves", solves);
